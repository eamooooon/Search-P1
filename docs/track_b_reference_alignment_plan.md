...
- Track A 比较模型自己的 `<plan>` 和 `<search>`，得到 `S_self`。
- Track B 比较模型自己的 `<search>` 和 `ground_truth.reference_steps`，得到 `S_ref`。
- Track B 不读取 `<plan>`。
- Track A 不读取 `reference_steps`。
- 当前实现只把 Track B 当作观测指标，不进入 `final_score`。
...
1. 拒绝采样：对同一问题采样多条完整 trajectory，只保留最终答案 EM 正确且包含合法 `<search>` 的轨迹。
