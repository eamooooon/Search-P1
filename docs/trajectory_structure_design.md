...
- `r_i` 是第 `i` 次工具调用前的推理，对应 `<think>...</think>`。
- `a_i` 是第 `i` 次搜索动作，对应 `<search>...</search>`。
- `o_i` 是环境返回的搜索结果，对应 `<information>...</information>`。
- `r_final` 是最终作答前的推理。
...
| `<think>...</think>` | 模型 | 工具调用或最终答案前的推理 |
| `<search>...</search>` | 模型 | 搜索动作，内容是 query |
| `<information>...</information>` | 环境 | 检索系统返回给模型的 observation |
...
- `<think>` 替换为 `<think>`。
- `<search>` 替换为 `<search>`。
- `<information>` 替换为 `<information>`。

内部实现仍可把 `<search>` 映射到现有 search action，但模型侧与 reward parser 只能看到 Search-P1 标签。
...
- 描述短而可执行的 search intent list，而不是 exact query list；每个 step 是一个可执行搜索目标，不要求等同于最终 `<search>` query。
...
- 缺失 `<plan>`。
- 出现多个 `<plan>`。
- planner 在 `<search>` 或 `<answer>` 后出现。
- planner 中没有 numbered `Step N: Search ...`。
- planner step 内嵌套 `<think>`、`<search>`、`<information>`、`<answer>` 等标签。
...
```text
<plan>p</plan>
(<think>r_i</think><search>a_i</search><information>o_i</information>)*
<think>r_final</think><answer>a_hat</answer>
```

约束：

- `<information>` 只能由环境注入，不能作为模型 action 解析。
- `<search>` 必须由模型输出，并由 rollout 环境执行。
- 每次 `<search>` 前必须有一个 `<think>`。
- 最终 `<answer>` 前必须有一个 `<think>`。
- `<answer>` 结束后不应再有 `<search>` 或 `<information>`。
...
- 向下一轮 rolling prompt 注入短控制指令，要求模型不要再输出 `<plan>`，继续输出一个 `<think>` 后接一个 `<search>` 或 `<answer>`。
- 该控制指令不是 `<information>`，不得写入最终用于 reward parsing 的序列化 trajectory。
...
- 后续每次搜索使用 `<think>` 加 `<search>`。
- 检索结果会由环境放入 `<information>`。
...
- 搜索返回必须包在 `<information>...</information>` 中。
- observation masking 也应使用 `<information>` 边界。
- invalid-action feedback 不是 `<information>`，应按现有控制文本路径处理。
...
- 校验 `<think>/<search>/<information>/<answer>` 状态顺序。
- 提取 planner steps 与 tool calls，供后续 path reward 使用。
- 当 `require_search_for_format == true` 时，零 `<search>` 的错误答案轨迹不能获得结构轨迹格式分。
...
- `extract_search_calls` 只读取模型输出的 `<search>` block，不能从 `<information>` 中提取 query。
- `count_actions` 等价于合法或可计数的 `<search>` 数量。
...
- `<information>` 内出现看似 query 的文本不会被计为 action。
- plan-only 第一阶段在非最终 rollout step 可接受，在最终 rollout step 不可接受。
- `require_search_for_format == true` 时，错误答案且没有 `<search>` 的轨迹不获得结构轨迹格式分。
